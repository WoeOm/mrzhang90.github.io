...
//            this.pic='img/j'+Math.ceil(Math.random()*8)+'.jpg';
    ...
    Lattic.prototype.rendering=function(){
        ...
        for(var i=0;i<len;i++){
            str+='<li style="background:url(img/j'+this.pic+'.jpg);background-position:'+(-i%3*100)+'px '+(-Math.floor(i/3)*100)+'px"><span style="display:none;">'+i+'</span></li>';
        }
        this.elem.html(str);
        ...
        this.emptyElem=li.eq(0).attr('index');
        this.keyEv();
//            this.mouseEv();
        this.showSerial();
        this.answer();
    ...
    //拖动时间
    Lattic.prototype.mouseEv=function(){
        var _this=this;
    ...
    Lattic.prototype.judge=function(tagIndex){
        ...
        $('.step').html(++this.step);
        var tmp=[8,0,1,2,3,4,5,6,7],arr_tmp=[];

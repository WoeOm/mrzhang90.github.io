...
function verif_screen(){
    return (window.innerWidth > window.innerHeight) ? false : true;
}
/** 设置meta viewport
 * 需要加这么一句<meta name="viewport" id="viewport" content="width=320, initial-scale=1, maximum-scale=1, user-scalable=no">
 * */
function setScale() {
    var viewport=document.getElementById('viewport');
    //var content ='width=320, initial-scale=1, maximum-scale=1, user-scalable=no';
...
function addClass_old(element,attr){
    attr=attr.trim();
    if(element.className){
        var old=element.className.trim();
        var arr=old.split(" ");
        if(arr.length==1){
            if(attr!=old)
                element.className=old+" "+attr;
        }else{
            var str='';
            for(var i in arr){
                if(arr[i]!=attr){
